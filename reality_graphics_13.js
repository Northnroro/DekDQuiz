for(var i=1;i<=32;i++) {
	$('#q'+i).append($('<br>'));
	var eee = $('<div style="width:175px; height:219px; clear:both;"></div>');
	$('#q'+i).append(eee);
	var ele = $('<img src="http://img-9gag-fun.9cache.com/photo/aOmj2AM_700b.jpg">');
	$(eee).append(ele);
	var topp = 161+Math.floor((i-1)/4)*219+Math.floor((i-1)/8)*21+Math.floor((i-1)/24)*7;
	var left = ((i-1)%4)*175;
	ele.css({
		"position": "absolute",
		"clip": "rect("+topp+"px,"+(left+175)+"px,"+(topp+219)+"px,"+left+"px)",
