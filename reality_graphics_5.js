for(var i=1;i<=32;i++) {
	$('#q'+i).append($('<br>'));
	var eee = $('<div width=175 height=217></div>');
	var ele = $('<img src="http://img-9gag-fun.9cache.com/photo/aOmj2AM_700b.jpg">');
	$('#q'+i).append(eee);
	eee.append(ele);
	var topp = 161+Math.floor((i-1)/4)*217+Math.floor((i-1)/8)*22+Math.floor((i-1)/24)*8;
	var left = ((i-1)%4)*175;
	ele.css({
		"position": "absolute",
		"clip": "rect("+topp+"px,"+(700-left+175)+"px,"+(1996-topp-217)+"px,"+left+"px)",
